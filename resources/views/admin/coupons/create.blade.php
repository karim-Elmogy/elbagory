...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="fas fa-tag"></i> إضافة خصم جديد</h2>
            ...
            <form action="{{ route('admin.coupons.store') }}" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" name="minimum_amount" class="form-control" step="0.01" min="0" value="{{ old('minimum_amount') }}">
                    </div>
                    
                    <div class="col-md-6 mb-3">
                        <label class="form-label">الحد الأقصى للخصم (ج.م) - للنسبة المئوية فقط</label>
                        <input type="number" name="maximum_discount" class="form-control" step="0.01" min="0" value="{{ old('maximum_discount') }}">
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" name="usage_limit_per_user" class="form-control" min="1" value="{{ old('usage_limit_per_user') }}">
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="customer_type" class="form-select" required>
                            <option value="all" {{ old('customer_type') === 'all' ? 'selected' : '' }}>الجميع</option>
                            <option value="retail" {{ old('customer_type') === 'retail' ? 'selected' : '' }}>قطاعي فقط</option>
                            <option value="wholesale" {{ old('customer_type') === 'wholesale' ? 'selected' : '' }}>جملة فقط</option>
                        </select>
                        @error('customer_type')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
